...
import { Terminal, Cpu, Menu, X, Activity, Award } from 'lucide-react';
import { useArchitecture } from '../context/ArchitectureContext';

export const Navbar = () => {
  const { isDevToolsActive, toggleDevTools, triggerTelemetry } = useArchitecture();
  ...
  return (
    <header className="sticky top-0 z-40 bg-[#080c14]/90 backdrop-blur-xl border-b border-slate-800/80 transition-all">
      <div className="container-custom flex items-center justify-between h-20">
        
        {/* Brand & System Status */}
        <a href="#home" className="flex items-center gap-3.5 group text-decoration-none">
          <div className="w-10 h-10 rounded-xl bg-cyan-950/80 border border-cyan-500/40 flex items-center justify-center text-cyan-400 group-hover:border-cyan-400 group-hover:shadow-[0_0_18px_rgba(6,182,212,0.45)] transition-all">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 bg-emerald-950/70 border border-emerald-500/40 px-2.5 py-0.5 rounded-full text-[10px] text-emerald-400 font-mono">
        ...
        {/* Action Controls: System Health & DevTools Switch */}
        <div className="hidden lg:flex items-center gap-3">
          <button
            onClick={handleHealthCheck}
            className="flex items-center gap-2 bg-slate-900/90 hover:bg-slate-800 border border-slate-700/80 text-slate-300 hover:text-cyan-400 px-3.5 py-1.5 rounded-lg text-xs font-mono transition-all"
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-mono border transition-all ${
              isDevToolsActive
                ? 'bg-cyan-950/70 border-cyan-500/50 text-cyan-300 shadow-[0_0_12px_rgba(6,182,212,0.25)]'
        ...
        <div className="md:hidden bg-slate-950/98 border-b border-slate-800 px-6 py-6 space-y-4 shadow-2xl animate-fade-in">
          ...
          <div className="pt-4 border-t border-slate-800 space-y-3">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-slate-900 border border-slate-700 text-slate-300 py-2.5 rounded-lg text-xs font-mono"
            >
