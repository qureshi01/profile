...
export const TelemetryButton = ({ traceData, label, className = "", icon: Icon = Terminal }) => {
  ...
  // When Telemetry is OFF: Keep button fully visible, but rendered in read-only API documentation mode!
  return (
    <div
      onClick={() => triggerTelemetry(traceData)}
      className={`group relative inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-950/90 border border-slate-800 text-slate-400 text-xs font-mono cursor-not-allowed opacity-90 hover:border-slate-700 transition-all ${className}`}
